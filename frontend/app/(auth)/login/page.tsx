...
const AuthCard = dynamic(() => import("@/components/auth/AuthCard"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[450px] rounded-xl border border-border bg-card flex items-center justify-center p-6">
      <div className="flex flex-col items-center gap-3">
        <div className="h-6 w-6 animate-spin rounded-full border-2 border-primary border-t-transparent" />
        <p className="text-[10px] text-text-muted">Loading login workspace...</p>
      </div>
    </div>
  ),
});
