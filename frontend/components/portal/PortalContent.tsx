    ...
    >
      ...
      >
        <form onSubmit={handleCreateTicket} className="space-y-4 text-left">
          {/* Email input is displayed when lookup hasn't run yet or user is logged out */}
          <Input
          ...
          <Input
            ...
            type="text"
            required
            placeholder="e.g. Broken zipper on leather jacket"
            value={newSubject}
          ...
          <div className="space-y-1">
            ...
            <textarea
              rows={4}
              required
              placeholder="Provide a detailed description so our AI support agent can immediately assist you..."
              value={newInitialMsg}
              onChange={(e) => setNewInitialMsg(e.target.value)}
              className={cn(
                "w-full rounded-xl border p-3 text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none transition-all",
                isDarkMode
          ...
          <div className="flex justify-end space-x-3 pt-3 border-t border-border/80 dark:border-slate-800/80">
            <Button
              type="button"
              variant="secondary"
              onClick={() => setIsNewTicketOpen(false)}
              className="h-9 text-xs rounded-lg px-4 border border-border"
            >
              Cancel
            </Button>
            <Button
